...
$connections = foreach ($connection in Get-NetTCPConnection | Where-Object {$_.State -in @('Listen','Established','SynSent','SynReceived')} | Select-Object -First 500) {
    ...
    [pscustomobject]@{name=[string]$owner.name;path=[string]$owner.path;pid=$pidValue;state=[string]$connection.State;local=('{0}:{1}' -f $connection.LocalAddress,$connection.LocalPort);remote=$remote;local_address=[string]$connection.LocalAddress;local_port=[int]$connection.LocalPort;remote_address=[string]$connection.RemoteAddress;remote_port=[int]$connection.RemotePort;direction=$(if($connection.State -eq 'Listen'){'INBOUND LISTENER'}else{'ACTIVE CONNECTION'})}
}
